...
| **Confidence** | `Trend`, `Premium` | **Neuro-Symbolic:**<br>Base 0.5 + Boosts based on Patient/Reliable mode rules.<br>Clamped `[0.10, 0.95]`. | **10s** (Polling) |
...
| :--- | :--- | :--- | :--- | :--- |
| **RBF** | Sender | `max(TargetRate, OriginalRate + 1.0) * TxVSize` | Must pay > original fee + relay cost. | **15s** (Polling) |
| **CPFP** | Receiver | `(TargetRate * (ParentSize + ChildSize)) - ParentFee` | Child pays deficit for Parent. | **15s** (Polling) |
